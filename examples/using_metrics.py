import importlib
import src.docs_ranking_metrics
...
if __name__ == "__main__":
    importlib.reload(src.docs_ranking_metrics.ranking_metrics)
    # Объявление метрик
    metrics = [ Bm25()]  # USE(), LaBSE(), MsMarcoCE(), MsMarcoST(), ]
    # Объявление класса агрегирующего обновление метрик
    rm = RankingMetrics(metrics, [2, 3])
    for item in data:
        '''
